...
\lecture{9 --- February 2, 2026}{Winter 2026}{Patrick Lam}{version 2}
...
\item \brac{expr} can be three things. Randomly choose \brac{term} \texttt{+} \brac{expr} and replace the original \brac{expr} by the chosen alternative.
\item Visit \brac{term}, with \texttt{+} and \brac{expr} still in our input being generated.
...
\begin{tikzpicture}[node distance=1.3cm]
  ...
  \node[bt,fill=green!20] (cd0) [below of=d0] {2};
  \node[bt] (e1) [right of=+,xshift=1cm] {\brac{expr}};
  ...
  \node[bt,fill=green!20] (cd1) [below of=d1] {3};
...
on a derivation tree is to expand a nonterminal into one of its alternatives (randomly-chosen); for instance, an \brac{expr}
can be an addition, subtraction, or just a term, and we need to replace the \brac{expr} with, for instance,
$\brac{term} + \brac{expr}$.

Which alternative? We'll start by randomly choosing an alternative (same as \texttt{simple\_grammar\_fuzzer}. But we'll want to use different strategies in the future.
\begin{python}
