...
to apply modern bounded model checking tools like CBMC and Kani,
...
Property foo.division-by-zero.2:
  ...
  !(x == 0u)
\end{verbatim}}
and with \verb+--show-vcc+ shows the relevant required verification conditions, as we discussed last time. Here's the report from the \texttt{y / x} division.
{\scriptsize \begin{verbatim}
...
\end{verbatim} }
We can see that CBMC is going to automatically put in an assertion checking for division by zero, and it knows
the conditions under which the y/x branch is executed, recording the conditions in variable guard1, which is true when $\neg (y \ge x)$. Verification succeeds on this branch if one can establish that input \texttt{x} is nonzero, given that guard1 is false (necessary to reach this branch).
...
Consider this program\footnote{https://model-checking.github.io/cbmc-training/cbmc/overview/debugging.html}:
\begin{lstlisting}[language=C]
#define SIZE 20
char buffer[SIZE];
...
(The Rust proptest crate starts with a random value and can ``shrink'' the value, but that's unlikely to get the
needed value.)
...
The assertions may contain refinements of the
postcondition that are supposed to be true due to what gets passed to the system in the arrange phase.
...
We have a struct \texttt{Inventory} with delegating methods implementing functionality:
\begin{lstlisting}[language=Rust]
...
values for \texttt{id} and \texttt{quantity} and check that the
...
Dafny verification is heavily based on contracts. Kani can also do contracts\footnote{\url{https://model-checking.github.io/kani/reference/experimental/contracts.html}}; the code is available in \texttt{code/L16/kani-contracts}.
...
\texttt{gcd} is correct. I tried to run it for 16-bit integers but it takes a lot longer and I gave up.
...
behaviour.  This includes bounds errors and overflows. (Overflows are
