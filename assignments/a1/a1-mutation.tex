...
\item [(c)] (5 points) Find a bug in \texttt{run\_length\_encoding} and provide a test case \texttt{test\_bugfix()} that fails on the provided version of the method but passes on a fixed version of the method. (We are not asking you to submit the fixed version.) Are your mutants from part (a) killed by the test case that also kills the bug? Again, write your answer in \texttt{mutants\_kill.py}, as well as a one or two-sentence description of the value of your mutant-killing tests from part (b), especially as compared to statement coverage; can you say that one is better?
\end{itemize}
